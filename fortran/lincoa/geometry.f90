...
integer(IK), allocatable :: rstat_(:)
integer(IK), allocatable :: rstat_2(:)
logical :: take_pgstp
real(RP) :: cstrv
real(RP) :: cvtol
...
allocate(rstat_(count(rstat >= 0)))
rstat_ = trueloc(rstat >= 0)
! Set FEASIBLE for the calculated S.
cstrv = maximum([ZERO, matprod(s, amat(:, rstat_)) - rescon(rstat_)])
feasible = (cstrv <= 0)
...
if (nact > 0 .and. gnorm > EPS .and. is_finite(gnorm)) then
    ...
    allocate(rstat_2(count(rstat == 1)))
    rstat_2 = trueloc(rstat == 1)
    cstrv = maximum([ZERO, matprod(pgstp, amat(:, rstat_2)) - rescon(rstat_2)])
