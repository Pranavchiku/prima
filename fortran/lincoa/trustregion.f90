...
integer(IK), allocatable :: resnew_(:)
integer(IK), allocatable :: resnew_2(:)
...
do iter = 1, maxiter  ! Powell's code is essentially a DO WHILE loop. We impose an explicit MAXITER.
    if (newact) then
        ...
        if (any(resact(1:nact) > 1.0E-4_RP * delta)) then
            ...
            if (resid > 0 .and. dd > EPS * delsq .and. .not. is_nan(ds)) then
                ...
                ad = -ONE
                allocate(resnew_(count(resnew > 0)))
                resnew_ = trueloc(resnew > 0)
                ad(resnew_) = matprod(dproj, amat(:, resnew_))
                frac = ONE
                allocate(resnew_2(count(ad > 0)))
                resnew_2 = trueloc(ad > 0)
                restmp(resnew_2) = resnew(resnew_2) - matprod(psd, amat(:, resnew_2))
    ...
    alphm = alpha
    ad = -ONE
    ad(resnew_) = matprod(d, amat(:, resnew_))
    frac = alpha
